...
<body>
  ...
  <section class="py-5">
    ...
    <h1>${q.qnaNo}번 게시글 상세보기</h1>
    <div>카테고리 : ${q.category}</div>
    <div>작성자 : ${q.writer}</div>
    <div>제목 : ${q.title}</div>
    <div>작성일 : ${q.createdAt}</div>
    <div>수정일 : ${q.modifiedAt}</div>
    <div>${q.content}</div>
    <form id="frm_remove" action="${contextPath}/qna/remove.do" method="post">
  ...
  <div id="edit_screen" class="px-4 px-lg-5 mt-5 text-center text-black">
    ...
    <h1>편집화면</h1>
    <form method="post" action="${contextPath}/qna/modify.do">
      <div>
        <label for="title">제목</label>
        <input type="text" id="title" name="title" value="${q.title}">
      </div>  
      <div>
        <label for="category">카테고리</label>
        <input type="text" id="category" name="category" value="${q.category}">
      </div>
      <div>
        <div><label for="content">내용</label></div>
        <textarea id="content" name="content">${q.content}</textarea>  <!-- summernote 편집기로 바뀌는 textarea -->
      </div>
      <div>
        <input type="hidden" name="qnaNo" value="${q.qnaNo}">
        <button>수정완료</button>
