...
$(function(){
    ...
    $('#recordPerPage').on('change', function(){
      location.href = '${contextPath}/qna/change/record.do?recordPerPage=' + $(this).val();  // session에 recorePerPage 올리기
    }) 
    ...
    $('.title').on('click', function(){
      location.href = '${contextPath}/qna/list.do?column=' + $(this).data('column') + '&order=' + $(this).data('order') + "&page=${page}";
    })
  })
  function fnNewWrite() {
    location.href='${contextPath}/qna/write.do';
  }
    ...
    <div>
      <table class="table">
        <thead class="text-center text-black">
           <tr>
              <th scope="col" data-column="QNA_NO" data-order="${order}">번호</th>
              <th scope="col" data-column="CATEGORY" data-order="${order}">카테고리</th>
              <th scope="col" data-column="WRITER" data-order="${order}">작성자</th>
              <th scope="col" data-column="TITLE" data-order="${order}">제목</th>
              <th scope="col" class="title" data-column="CREATED_AT" data-order="${order}">날짜</th>
              <th scope="col" class="title" data-column="HITS" data-order="${order}">조회수</th>
             </tr>
        </thead>
        ...
        <tbody class="text-center text-black">
	        ...
	        <c:forEach items="${qnaList}" var="qna" varStatus="vs">
	          <c:if test="${qna.state == 1}">
	            ...
	            <tr class="list">
	              ...
	              <td>${qna.category}</td>
	              <td>${qna.writer}</td>
	              <td><a href="${contextPath}/qna/detail.do?qnaNo=${q.qnaNo}" >${q.title}</a></td>
	              <td>${qna.createdAt}</td>
	              <td>${qna.hits}</td>
	              ...
	              <td>
                <!-- DEPTH에 의한 들여쓰기 -->
                <c:forEach begin="1" end="${qna.depth}" step="1">&nbsp;&nbsp;&nbsp;</c:forEach>
                <!-- 답글은 [Re] 표시하기 -->
                <c:if test="${qna.depth > 0}">[Re]</c:if>
                <!-- 제목 -->
                ${qna.title}
                <!-- 답글작성하기 버튼 -->
              ...
              <td>${qna.createdAt}</td>
              <td>
                <form class="frm_remove" method="post" action="${contextPath}/qna/remove.do">
            ...
            <tr class="write blind">
              <td colspan="6">
                <form method="post" action="${contextPath}/qna/reply/add.do">
                  <div>
